...
function SeeAllGradelist({ handleSeeAllModal, data, index }) {
        ...
        <>
            <div className='fixed inset-0 z-20 bg-black bg-opacity-80 flex justify-center items-center'>
                <div className='relative sm:max-w-[50%]  max-h-[90vh] flex justify-center items-center p-4 mt-[64px]'>
                    {/* Card container */}
                    <div className={`${data.AdmissionOpen ? 'bg-gradient-to-b from-green-100 via-green-200 to-green-300': 'bg-red-200' } w-full border rounded-lg flex flex-col gap-4 shadow-md p-7 m-4 
                        ...
                        <h3 className="text-2xl font-semibold text-red-950 mt-2">
                            {(`${index + 1}.   ${data?.GradeName}`)}
                        </h3>
                        <div className='flex flex-col gap-1'>
                            <p className=" mt-2"><b>Grade Name:</b> {data?.GradeName}</p>
                            <p className=" mt-2"><b>Area Name:</b> {data?.AreaName}</p>
                            <p className=" mt-2"><b>Level Name:</b> {data?.LevelName}</p>
                            <p className=" mt-2"><b>Theory Pass Percent:</b> {data?.TheoryPassPercent}</p>
                            <p className=" mt-2"><b>Practical Pass Percent:</b> {data?.PracticalPassPercent}</p>
                            <p className=" mt-2"><b>GradeSheet Caption:</b> {data?.GradeSheetCaption}</p>
                            <p className=" mt-2"><b>Mark Sheet Caption:</b> {data?.MarkSheetCaption}</p>
                            <p className=" mt-2"><b>Parent as Bill Payer:</b> {data?.ParentAsBillPayer ? "Yes" : "No"}</p>  
                            <p className=" mt-2"><b>Admission Open:</b> {data.AdmissionOpen ? "Yes": "No"}</p>
...
export default SeeAllGradelist;
